...
public class PassController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		/*
		http://localhost:8585/K08JSPServlet/mvcboard/view.do?idx=137
		요청명은 이와 같으므로 파하미터를 받아오는 작업을 해야한다.
		서블릿에서는 2가지 방법으로 받을 수 있는데.
		방법1 : request내장객체를 이요한다 =>req.getParmater(파라미터명
		...
		if(confirmed) {
			if(mode.equals("edit")) {
				...
				*/
				...
				검증없이 진입해였다면 세션에 저장된 패스워드가 없을것이므로
				수정 처리가 되지 않도록 처리할것이다.
				*/
				resp.sendRedirect("../mvcboard/edit.do?idx="+idx);
				
			}
			else if(mode.equals("delete")) {
				//게시물을 삭제하고 첨부파일 있는경우 같이 삭제한다.
				dao = new MVCBoardDAO();
				//기존 게시물의 내용을 읽어온다.
